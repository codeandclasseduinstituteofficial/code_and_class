...
const CourseDetailed = () => {
  ...
  const rawPrice = Number(course?.price?.toString?.().replace?.(/[^\d.]/g, '')) || 0;
  const rawDiscountPct = Number(course?.discount?.toString?.().replace?.(/[^\d.]/g, '')) || 0;
  const originalPrice = rawDiscountPct ? Math.round(rawPrice + (rawPrice * rawDiscountPct) / 100) : rawPrice;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="card-surface p-6 sticky top-24 space-y-5 border-2 border-brand-100 shadow-lg">
                ...
                <Checkout courseId={course?._id} courseTitle={course?.title} courseType={course?.courseType} />
